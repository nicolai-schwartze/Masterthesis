...
from memeticJADE import memeticJADE

class OptAlgoMemeticJADE(IOptAlgoBase):
    """
    Implementation of the IOptAlgoBase interface. 
    It provides a wrapper for the memeticJADE optimisation algorithm
    ...
    def opt(self, function):
        return memeticJADE(self.init_guess, function, self.min_err, self.max_iter)
...
if __name__ == "__main__": 
    ...
    max_iter = 10**3
    min_err = 10**(-200)
    mJade = OptAlgoMemeticJADE(initialPop, max_iter, min_err)
    print(mJade.max_iter)
    print(mJade.min_err)
    print(mJade.init_guess)
    
    def sphere(x):
        return np.dot(x,x)
    
    (popDynamic, FEDynamic, FDynamic, CRDynamic) = mJade.opt(sphere)
