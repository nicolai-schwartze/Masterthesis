...
class FemPde3(FemPdeBase):
    """
    **Implementation of PDE3 of the testbed:** 
    ...
    **with the solution:** 
    ...
    def __init__(self, show_gui, max_ndof=50000):
        ...
        # init protected
        self._pde_string = "laplacian(u(x)) = 4"
        self._ngs_ex = ngs.x**2 + ngs.y**2 + ngs.x + ngs.y + 1
...
if __name__ == "__main__":
    ...
    fempde3 = FemPde3(True)
    print(fempde3.pde_string)
    
    try:
        fempde3.exact(np.array([0.5,0.5]))
    except:
        print("Î error message above")
    
    try:
        fempde3.approx(np.array([0.5,0.5]))
    except:
        print("Î error message above")
    ...
    print("exact(0.5, 0.5) = {}".format(fempde3.exact(np.array([0.5,0.5]))))
    print("approx(0.5, 0.5) = {}".format(fempde3.approx(np.array([0.5,0.5]))))
    print("L2 norm to the real solution {}".format(fempde3.normL2()))
    print("solving took {} sec".format(fempde3.exec_time))
    print("solving uses {} Mb".format(fempde3.mem_consumption/1000000))
    ...
    ax.set_ylabel("y")
    ax.set_zlabel("z")
    ax.set_title("Solution of PDE 3", fontsize=20)
    plt.show()
    fig.savefig("sol_pde_3.pdf", bbox_inches='tight')
