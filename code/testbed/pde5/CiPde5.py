# -*- coding: utf-8 -*-
"""
Created on Fri May 15 13:50:03 2020
    ...
    **with the solution:** 
    ...
    def fitness_func(self, kernels): 
        ...
        for i in range(len(self._nc)):
            ...
            f = -20/((1 + 400*(np.sqrt((x - 0.05)**2 + (y - 0.05)**2) -0.7)**2)*np.sqrt((x - 0.05)**2 + (y - 0.05)**2)) \
                + (16000*(np.sqrt((x - 0.05)**2 + (y - 0.05)**2) -0.7))/((400*(np.sqrt((x - 0.05)**2 + (y - 0.05)**2) -0.7)**2 + 1)**2)
...
if __name__ == "__main__":
    ...
    cipde5 = CiPde5(mJade, gkernel, nb, nc)
    
    print(cipde5.pde_string)
    
    try:
        cipde5.exact(np.array([0.0,0.0]))
    except:
        print("Î error message above")
    
    try:
        cipde5.approx(np.array([0.0,0.0]))
    except:
        print("Î error message above")
    ...
    print("exact(0.5, 0.5) = {}".format(cipde5.exact(np.array([0.5,0.5]))))
    print("approx(0.5, 0.5) = {}".format(cipde5.approx(np.array([0.5,0.5]))))
    print("L2 norm to the real solution {}".format(cipde5.normL2()))
    print("solving took {} sec".format(cipde5.exec_time))
    print("solving uses {} Mb".format(cipde5.mem_consumption/1000000))
