# -*- coding: utf-8 -*-
"""
Created on Thu Apr 30 15:28:58 2020
...
class FemPde8(FemPdeBase):
    """
    **Implementation of PDE8 of the testbed:** 
        
    .. math:: 
        \Delta u(\mathbf{x}) = 1/(x^2 + y^2 - x - y + 0.5)^{1/2}
    ...
    **with the solution:** 
    ...
    .. math:: 
        u(\mathbf{x}) = ((x-0.5)^2 + (y-0.5)^2)^{1/2}
    ...
    def __init__(self, show_gui, max_ndof=50000):
        ...
        # init protected
        self._pde_string = "laplacian(u(x)) = 1/(x^2 + y^2 - x - y + 0.5)^(1/2)"
        self._ngs_ex = ngs.sqrt((ngs.x-0.5)**2 + (ngs.y-0.5)**2)
    ...
    def solve(self): 
        ...
        self._f += (-1/ngs.sqrt(ngs.x**2 + ngs.y**2 - ngs.x - ngs.y + 0.5))*v*ngs.dx
...
if __name__ == "__main__":
    ...
    fempde8 = FemPde8(True)
    print(fempde8.pde_string)
    
    try:
        fempde8.exact(np.array([0.0,0.0]))
    except:
        print("Î error message above")
    
    try:
        fempde8.approx(np.array([0.0,0.0]))
    except:
        print("Î error message above")
    ...
    print("exact(0.5, 0.5) = {}".format(fempde8.exact(np.array([0.5,0.5]))))
    print("approx(0.5, 0.5) = {}".format(fempde8.approx(np.array([0.5,0.5]))))
    print("L2 norm to the real solution {}".format(fempde8.normL2()))
    print("solving took {} sec".format(fempde8.exec_time))
    print("solving uses {} Mb".format(fempde8.mem_consumption/1000000))
    ...
    ax.set_ylabel("y")
    ax.set_zlabel("z")
    ax.set_title("Solution of PDE 8", fontsize=20)
    plt.show()
    fig.savefig("sol_pde_8.pdf", bbox_inches='tight')
