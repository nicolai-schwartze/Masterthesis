...
\definecolor{key-color}{RGB}{100,0,255}

\lstset{language=Python, emph={None, bool, dict, float, string}, emphstyle={\color{key-color}} }
...
Although the post processing block is not actually a class, it is still represented in this diagram. This module provides funtions that take \inlinecode{FemPdeN} or \inlinecode{CiPdeN} objects and performs actions with them. \todo{update implemented function description and class diagram in appendix}

\begin{itemize}
	\item 
	\inlinecode{bool saveExpObj(obj, filename)} \\
	Save an \inlinecode{CiPdeN} object as a \gls{json} file. The filename parameter can include a path, but it must end with .json. The results: execution time, memory usage, solution quality and all intergenerational data of the optimisation algoritm are stored in the file. 
	...
	\item 
	\inlinecode{None plotApprox3D(kernel, parameter, lD, uD)} \\
	...
	This function is a wrapper for the \inlinecode{scipy.stats.wilcoxon} (\cite{scipy_scipystatswilcoxon_2020}). A string is returned, that describes if the mean/median of a is significantly smaller than the mean/median of b. The default significance level is set to 0.05. 
\end{itemize}
