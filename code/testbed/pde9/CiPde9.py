# -*- coding: utf-8 -*-
"""
Created on Sat May 16 20:35:46 2020
...
class CiPde9(CiPdeBase):
    ...
    **with the solution:** 
    ...
    def exact(self, x): 
        y = x[1]
        x = x[0]
        return np.arctan(20*((x + y)/(2**(1/2)) -0.8))*x*(1-x)*y*(1-y)
    ...
    def fitness_func(self, kernels): 
        ...
        for i in range(len(self._nc)):
            ...
            f = (20*np.sqrt(2)*(x**2 + y**2 -2*x**2*y - 2*x*y**2 + 4*x*y - x - y))/(400*((x+y)/(np.sqrt(2))-0.8)**2+1) + \
                (16000*(1-x)*x*(1-y)*y*((x+y)/(np.sqrt(2))-0.8))/((400*((x+y)/(np.sqrt(2))-0.8)**2+1)**2) + \
                np.arctan(20*((x+y)/(np.sqrt(2))-0.8))*(2*(1-y)*y + 2*(1-x)*x)
...
if __name__ == "__main__":
    ...
    cipde9 = CiPde9(mpJADE, gkernel, nb, nc)
    
    print(cipde9.pde_string)
    
    try:
        cipde9.exact(np.array([0.0,0.0]))
    except:
        print("Î error message above")
    
    try:
        cipde9.approx(np.array([0.0,0.0]))
    except:
        print("Î error message above")
    ...
    print("exact(0.5, 0.5) = {}".format(cipde9.exact(np.array([0.5,0.5]))))
    print("approx(0.5, 0.5) = {}".format(cipde9.approx(np.array([0.5,0.5]))))
    print("L2 norm to the real solution {}".format(cipde9.normL2()))
    print("solving took {} sec".format(cipde9.exec_time))
    print("solving uses {} Mb".format(cipde9.mem_consumption/1000000))
